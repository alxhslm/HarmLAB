function [results,curr] = hbm_bb(hbm,problem,A0,w0,X0,AEnd,wEnd,XEnd)
problem.type = 'bb';
...
switch hbm.cont.method
    case 'none'
        ...
        while Asol(end) <= AMax && Asol(end) >= AMin
            ...
            if ~any(isnan(curr(end).z))
                ...
                hbm_bb_plot('err',hbm,problem,curr(end));
    ...
    case 'predcorr'
       ...
       prog.NIter = 0;
        ...
        curr = hbm_bb_results(Zprev,Tprev,pred,corr,hbm,problem);
        results = curr;
        
        hbm_bb_plot('init',hbm,problem,init);
        fprintf('STEP    PRED    CORR   STATUS  INFO     ITER    TOT     FREQ       AMP         ')
        fprintf('Z(%d)       ',1:length(z0))
        fprintf('\n')       
        
        fprintf('%3d   %6.4f  %6.4f     %s      %3s     %3d    %3d    %6.2f    %6.2f      ',prog.NStep,pred.step,corr.step,'S','Ini',corr.it,prog.NIter,results(end).w,results(end).A)
        ...
        while norm(Zprev - Zend) > hbm.cont.max_step
            ...
            switch hbm.cont.predcorr.corrector
                case 'pseudo'
                    ...
                    while corr.it <= hbm.cont.predcorr.maxit
                        Zlast = Z;
                        J = hbm_bb_jacobian(Z,hbm,problem);
                        F = hbm_bb_constraints(Z,hbm,problem);
            ...
            if  bConverged && curr(end).sCorr >= hbm.cont.min_step && curr(end).sCorr <= hbm.cont.max_step && curr(end).A > 0
                %success
                status = 'S';
                hbm_bb_plot('data',hbm,problem,curr(end));
            ...
            fprintf('%3d   %6.4f  %6.4f     %s      %3s     %3d    %3d    %6.2f    %6.2f      ',prog.NStep,curr(end).sPred,curr(end).sCorr,status,info,corr.it,prog.NIter,curr(end).w,curr(end).A)
...
x = packdof(curr.X,hbm.harm.iRetain);
u = packdof(curr.U);
w = curr.w;
curr.H = hbm_objective('complex',hbm,problem,w*hbm.harm.rFreqRatio,x,u);
