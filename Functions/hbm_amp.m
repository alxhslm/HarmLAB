...
if isfield(problem,'xscale')
    ...
    problem.Ycale = [problem.Xscale; problem.Ascale];
    problem.Zscale = [problem.Xnlscale; problem.Ascale];
       ...
       prog.NIter = 0;
        ...
        while norm(Yprev - Yend) > hbm.cont.max_step
            ...
            if  bConverged && curr(end).sCorr >= hbm.cont.min_step && curr(end).sCorr <= hbm.cont.max_step && curr(end).A > 0
                ...
                Ysol = ysol./(repmat(problem.Yscale,1,size(ysol,2)));
...
xnl = Z(1:end-1).*problem.Xnlscale;
...
Y = y./problem.Yscale;
dYdZ = blkdiag(R .* (problem.Xnlscale(:)'./problem.Xscale(:)),1);
